...
%   distmesh2d.m v1.1
%   Modifications by David Silvester
...
rng(26121958)                                        % reproducible random numbers | DJS change
...
while count<301,                                     % set 300 iteration limit | DJS change
  count=count+1;
  ...
  if max(sqrt(sum((p-pold).^2,2))/h0)>ttol           % Any large movement?
    ...
    if count<30 | rem(count,5)==0,                   % selective output | DJS change
